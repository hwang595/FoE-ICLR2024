...
# MoE
parser.add_argument('--k', default=1, type=int, help="select top k")
...
def test_local(net, testloader, client_idx, prefix=''):
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            outputs = net.forward_expert(images, client_idx)
    ...
    for classname, correct_count in correct_pred.items():
        ...
        print(f'Local Acc. - class: {classname:5s} is {accuracy:.1f} %')
        
    print("Dataset: {}; Valid. Acc.: {:.2f}".format(
            prefix, 100 * correct / total))


class LocalUpdatePub(object):
    def __init__(self, args, dataset=None, idxs=None, testloader=None):
        self.args = args
    ...
    def train(self, net, client_idx, fl_round=0):
        ...
        for epoch in range(args.local_ep):
            ...
            print('Client: {}, FL Round {:3d}, Local train loss {:.3f}'.format(client_idx, fl_round, epoch_loss[-1]))
        return net.feat_map.state_dict(), sum(epoch_loss) / len(epoch_loss)

    def test(self, net, client_idx):
        test_local(net, self.testloader, client_idx, 'CIFAR10')
...
def test(net, testloader, prefix=''):
    ...
    for classname, correct_count in correct_pred.items():
        accuracy = 100 * float(correct_count) / total_pred[classname]
        print(f'Global Acc. - class: {classname:5s} is {accuracy:.1f} %')
...
train_id_list, _ = cifar_synthetic(trainset, args.num_clients)
...
for fl_round in range(args.fl_rounds):
    print("FL round: {}, current lr: {}".format(fl_round, args.lr))
    w_locals, loss_locals = [], []
    for local_id in range(args.num_clients):
        local = LocalUpdatePub(args, trainset, train_id_list[local_id], testloader=testloader)
        ...
        if fl_round % 20 == 0 and fl_round != 0:
            local.test(net=net_local_list[local_id], client_idx=local_id)
