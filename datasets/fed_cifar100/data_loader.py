...
def get_dataloader(data_dir, train_bs, test_bs, client_idx=None):
    ...
    if client_idx is None:
        ...
        temp_train_x = []
        temp_train_y = []
        for idx in range(client_idx * 50, client_idx * 50 + 50):
            temp_train_x.append(train_h5[_EXAMPLE][str(idx)][_IMGAE][()])
            temp_train_y.append(train_h5[_EXAMPLE][str(idx)][_LABEL][()])
        train_x = np.vstack(temp_train_x)
        train_y = np.hstack(temp_train_y).squeeze()
    ...
    # preprocess
    train_x = utils.preprocess_cifar_img(torch.from_numpy(train_x), train=True)
    train_y = torch.tensor(train_y)
    if len(test_x) != 0:
        test_x = utils.preprocess_cifar_img(torch.from_numpy(test_x), train=False)
...
if __name__ == "__main__":
    ...
    data_local_num_dict, train_data_local_dict, test_data_local_dict, class_num = load_partition_data_federated_cifar100(
        data_dir="./datasets", batch_size=128)
