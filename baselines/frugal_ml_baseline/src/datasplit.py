...
def data_split(datapath = '/Users/lingjiaochen/Documents/projects/MLMarket/MLMarket/MLAS/FER-2013/mlserviceperformance_FERPlus_Full/', 
    ...
    if(perclasssplit):
        ...
        #                                   train_ratio = train_ratio, 
        #                                   randseed = randseed)
        trainindex,testindex = generate_index_rulebased(datapath = datapath,
                                          train_ratio = train_ratio, 
                                          randseed = randseed)
...
def generate_index_rulebased(datapath = '/Users/lingjiaochen/Documents/projects/MLMarket/MLMarket/MLAS/FER-2013/mlserviceperformance_FERPlus_Full/', 
                   train_ratio ='0.5', 
                   randseed = 0,):        
    allpath = sorted(glob.glob(datapath+'*.txt'))
    data_num = len(np.loadtxt(allpath[0], dtype=str))
    # generate random index
    vanilla_index = np.arange(data_num)
    trainlast = int(data_num*train_ratio)
    trainindex = vanilla_index[0:trainlast]
    testindex = vanilla_index[trainlast:]
